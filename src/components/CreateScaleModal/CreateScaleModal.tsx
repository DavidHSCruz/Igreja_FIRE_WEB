...
import { FaTimes, FaCalendarAlt, FaCheck, FaPlus, FaTrash, FaSearch } from 'react-icons/fa';
...
import { ScaleGroup } from '../ScaleCard/ScaleCard';
...
export const CreateScaleModal = ({ isOpen, onClose, contextId, initialData }: CreateScaleModalProps) => {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            const sortedEventos = eventosRes.data.sort((a: Evento, b: Evento) => 
            ...
            if (initialData) {
                ...
                const foundEvent = sortedEventos.find((e: Evento) => {
                     if (initialData.eventoId && e.id === initialData.eventoId) return true;

                     const eventDate = new Date(e.dataInicio).toISOString().split('T')[0];
                     const initDate = new Date(initialData.data).toISOString().split('T')[0];
                ...
                const initialRows: ScaleRow[] = sourceItems.map((item: any) => {
                    const roleName = item.role || item.roles; // Handle both singular (original) and plural (merged)
                    const atividade = fetchedAtividades.find((a: Atividade) => a.name === roleName);
    ...
    const handleRowChange = (rowId: string, field: keyof ScaleRow, value: string) => {
    ...
    const getFilteredMembers = (term: string) => {
        ...
        return membros.filter(m => 
            ...
            m.atuacoes?.some(a => a.atividade?.name.toLowerCase().includes(lowerTerm))
        );
    };
    ...
    const handleDelete = async () => {
        ...
        const confirmDelete = window.confirm("Tem certeza que deseja excluir esta escala? Todas as atividades vinculadas a ela serão removidas.");
        ...
        try {
            ...
            const originalItems = initialData.originalItems || initialData.items;
            ...
            for (const id of scalesToDelete) {
                try {
                    ...
                    const itemsWithThisScale = originalItems.filter((item: any) => item.scaleId === id);
                    for (const item of itemsWithThisScale) {
                        if (item.membroId) {
                            try {
                                await api.delete(`/escalas/${id}/voluntarios/${item.membroId}`);
                            } catch (e) {
                                console.warn(`Failed to delete volunteer ${item.membroId} from scale ${id}`, e);
                            }
                        }
                    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const originalScales = new Map<string, { id: string, activityId: string, memberIds: Set<string> }>();
            ...
            for (const [activityId, newMembers] of newScales.entries()) {
                ...
                if (targetScaleId) {
                    ...
                    try {
                        ...
                        // Sync Volunteers
                        const originalMembers = originalScales.get(targetScaleId)!.memberIds;

                        // Remove members that are no longer in this activity
                        for (const oldMemberId of originalMembers) {
                            if (!newMembers.has(oldMemberId)) {
                                await api.delete(`/escalas/${targetScaleId}/voluntarios/${oldMemberId}`)
                                    .catch(e => console.warn(`Failed to remove volunteer ${oldMemberId}`, e));
                            }
                        }
                        ...
                        for (const newMemberId of newMembers) {
                            if (!originalMembers.has(newMemberId)) {
                                await api.post(`/escalas/${targetScaleId}/voluntarios`, { membroId: newMemberId });
                            }
                        }
                    } catch (e) {
                    ...
                    try {
                        ...
                        for (const memberId of newMembers) {
                            await api.post(`/escalas/${newScaleId}/voluntarios`, { membroId: memberId });
                        }
                    } catch (e) {
                        console.error(`Failed to create scale for activity ${activityId}`, e);
                    }
                }
            }
                ...
                if (!processedScaleIds.has(id)) {
                    try {
                        ...
                        for (const memberId of data.memberIds) {
                            await api.delete(`/escalas/${id}/voluntarios/${memberId}`)
                                .catch(e => console.warn(`Failed to clear volunteer ${memberId} from deleted scale`, e));
                        }
                        // Delete scale
                        await api.delete(`/escalas/${id}`);
                    } catch (e) {
            ...
            alert(error.response?.data?.message || "Erro ao salvar escalas.");
        } finally {
            setLoading(false);
        }
    };
                                ...
                                >
                                    ...
                                    {eventos.map(evento => (
                                        <option key={evento.id} value={evento.id}>
                                            {new Date(evento.dataInicio).toLocaleDateString()} - {evento.nome}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            {selectedEventId && (
                                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="text-xs text-gray-400 bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-lg mb-4">
                                        <span className="text-yellow-500 font-bold">Nota:</span> Novas escalas são criadas como <strong className="text-yellow-500">PENDENTE</strong>. O membro deve confirmar sua participação.
                                    </div>

                                    <div className="space-y-3">
                                        {rows.map(row => (
                                            <div key={row.id} className="bg-[#0F0F0F] p-4 rounded-xl border border-white/5 space-y-3 relative">
                                                <div className="flex gap-3">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <select
                                                            ...
                                                            onChange={(e) => handleRowChange(row.id, 'atividadeId', e.target.value)}
                                                    ...
                                                    <div className="flex-[1.5] relative">
                                                        <label className="text-xs text-gray-500 mb-1 block">Voluntário</label>
                                                        <div className="relative">
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={(e) => handleRowChange(row.id, 'searchTerm', e.target.value)}
                                                        ...
                                                        {openDropdownRowId === row.id && row.searchTerm && (
                                                            <div className="absolute z-10 left-0 right-0 mt-1 bg-[#1A1A1A] border border-white/10 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                                                {getFilteredMembers(row.searchTerm).length > 0 ? (
                                                                    getFilteredMembers(row.searchTerm).map(m => (
                                                                        <button
                                                                            key={m.id}
                                                                        ...
                                                                        >
                                                                            <div className="w-6 h-6 rounded-full bg-gray-700 overflow-hidden flex-shrink-0">
                                                                                {m.user?.avatarUrl ? (
                                                                                    <img src={m.user.avatarUrl} alt={m.nome} className="w-full h-full object-cover" />
                                                                                ) : (
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <div className="text-xs text-gray-500 truncate max-w-[180px]">
                                                                                    {m.atuacoes?.map(a => a.atividade.name).join(', ')}
                                                                                </div>
                                                                            </div>
                                                                        </button>
                                                                    ))
