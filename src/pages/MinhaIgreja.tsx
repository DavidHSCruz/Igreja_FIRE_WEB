...
const toSelectValue = (value: unknown, fallback: string): string => {
...
export const MinhaIgreja = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "membros" | "celulas" | "eventos" | "ministerios" | "areas"
  >("membros");
  ...
  const [entityFormData, setEntityFormData] = useState<Record<string, unknown>>(
    {},
  );
  ...
  const openEditModal = (item: BaseEntity) => {
  ...
  const handleSaveEntity = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const igrejaId = user?.membro?.igreja?.id;
      ...
      if (activeTab === "celulas") {
        const diaSemanaValue = dataToSend.dia_semana;
        if (
          typeof diaSemanaValue === "string" ||
          typeof diaSemanaValue === "number"
        ) {
          dataToSend.dia_semana = parseInt(String(diaSemanaValue), 10);
        }
      }
      ...
      alert(getApiErrorMessage(error) || "Erro ao salvar item.");
    }
  };
  ...
  const openUserModal = (membro: Membro) => {
    ...
    setUserFormData({
      email: toStringValue(membro.email),
      password: "",
      system_role: "MEMBRO",
    });
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(getApiErrorMessage(error) || "Erro ao criar usuário.");
    }
  };
  ...
  const handleUnlinkUser = async (membro: Membro) => {
    ...
    if (
      !window.confirm(
        `Tem certeza que deseja desvincular o usuário ${membro.user.email} deste membro?`,
      )
    )
      return;
    ...
    try {
      ...
      alert(getApiErrorMessage(error) || "Erro ao desvincular usuário.");
    }
  };
          ...
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="bg-[#202020] text-xs uppercase font-bold text-gray-400">
              <tr>
                ...
                {activeTab === "membros" && (
                  <th className="px-6 py-4">Usuário Vinculado</th>
                )}
                {activeTab === "celulas" && (
            ...
            <tbody className="divide-y divide-white/5">
              {filteredData.map((item) => (
                ...
                >
                  ...
                  {activeTab === "celulas" && (
                    <td className="px-6 py-4">
                      {toStringValue(item.lider) || "N/A"}
                    </td>
                  )}

                  {activeTab === "eventos" && (
                    <td className="px-6 py-4">
                      {toStringValue(item.dataInicio)
                        ? new Date(
                            toStringValue(item.dataInicio),
            ...
            <form onSubmit={handleSaveEntity} className="p-6 space-y-4">
              ...
              {activeTab === "membros" && (
                <>
                  <ProfileInput
                    label="Nome Completo"
                    value={toStringValue(entityFormData.nome)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                        nome: e.target.value,
                  ...
                  <ProfileInput
                    label="Email"
                    type="email"
                    value={toStringValue(entityFormData.email)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <ProfileInput
                      ...
                      type="date"
                      value={toDateOnlyValue(entityFormData.dataNascimento)}
                      onChange={(e) =>
                        setEntityFormData({
                          ...entityFormData,
                    ...
                    <ProfileInput
                      label="Data Entrada"
                      type="date"
                      value={toDateOnlyValue(entityFormData.dataEntrada)}
                      onChange={(e) =>
                        setEntityFormData({
                          ...entityFormData,
                  ...
                  <ProfileInput
                    label="Telefone"
                    value={toStringValue(entityFormData.telefone)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                  ...
                  <ProfileInput
                    label="Endereço"
                    value={toStringValue(entityFormData.endereco)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
              ...
              {activeTab === "celulas" && (
                <>
                  ...
                  <ProfileInput
                    label="Líder (Nome)"
                    value={toStringValue(entityFormData.lider)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        value={toSelectValue(entityFormData.dia_semana, "0")}
                        onChange={(e) =>
                          setEntityFormData({
                            ...entityFormData,
                    ...
                    <ProfileInput
                      label="Horário"
                      type="time"
                      value={toStringValue(entityFormData.horario)}
                      onChange={(e) =>
                        setEntityFormData({
                          ...entityFormData,
                  ...
                  <ProfileInput
                    label="Local"
                    value={toStringValue(entityFormData.local)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
              ...
              {activeTab === "eventos" && (
                <>
                  ...
                  <ProfileInput
                    label="Data Início"
                    type="datetime-local"
                    value={toStringValue(entityFormData.dataInicio)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      value={toStringValue(entityFormData.descricao)}
                      onChange={(e) =>
                        setEntityFormData({
                          ...entityFormData,
              ...
              {(activeTab === "ministerios" || activeTab === "areas") && (
                <>
                  <ProfileInput
                    label="Nome"
                    value={toStringValue(entityFormData.name)}
                    onChange={(e) =>
                      setEntityFormData({
                        ...entityFormData,
                        name: e.target.value,
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      className="w-full bg-[#252525] border border-white/10 rounded-lg py-3 px-4 text-sm text-white focus:outline-none focus:border-secondary transition-colors h-24 resize-none"
                      value={toStringValue(entityFormData.description)}
                      onChange={(e) =>
                        setEntityFormData({
                          ...entityFormData,
